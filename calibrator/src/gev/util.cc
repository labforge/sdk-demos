...
#include "io/util.hpp"
#include <opencv2/opencv.hpp> // toUpperCase

using namespace std;
using namespace cv;

PvStream *labforge::gev::OpenStream( const PvString &aConnectionID, PvResult *res ) {
  PvStream *lStream;

  PvSystem lSystem;
  lSystem.Find();

  PvDeviceInfo*dev = nullptr;
  for(size_t i = 0; i < lSystem.GetInterfaceCount(); i++) {
    const PvInterface *iface = lSystem.GetInterface(i);
    for(size_t j = 0; j < iface->GetDeviceCount(); j++) {
      dev = (PvDeviceInfo*)iface->GetDeviceInfo(j);
      if(toUpperCase(string(dev->GetUniqueID().GetAscii())) == toUpperCase(string(aConnectionID.GetAscii()))) {
        break;
      } else {
        dev = nullptr;
      }
  ...
  lStream = PvStream::CreateAndOpen( dev, res );
