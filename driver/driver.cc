...
@file driver.cc Sample installation script for eBUS driver
...
static void ToLower( string &aString );
...
EB_APP_STATUS ListNetworkAdapters()
{
    ...
    for ( uint32_t i = 0; i < lInstaller.GetNetworkAdapterCount(); i++ )
    {
        ...
        else
        {
            ...
            {
                cout << "                        " << lIPAddress[ j ].GetAscii() << " | " << lSubnetMask[ j ].GetAscii() << endl;
            }
...
{
    EbNetworkAdapter* lAdapter = NULL;
    uint32_t lCount = aInstaller.GetNetworkAdapterCount();
...
{
    string::iterator lIt = aString.begin();
    while ( lIt != aString.end() )
    {
        *lIt = static_cast<char>( ::tolower( *lIt ) );
        lIt++;
    }
}